...
function OLECATAPage(_pageManager, _pageTemplateRenderer, _pageConfig, _audioContext, _bufferSize, _audioFileLoader, _stimulus, _session, _errorHandler, _language) {
  ...
  for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
    ...
    if (element.type === "long_text") {
      table.append($("<tr><td colspan='2'><textarea name='" + element.name + "' id='" + element.name + "'></textarea></td></tr>"));
      ...
      table1.append($("<tr><td colspan=3 id=" + element.name + " style='vertical-align:top; padding-top:" + $('#feedback').css('margin-top') + "'><br><strong>" + element.label + "</strong><br>" + element.content + "<br><br></td></tr>"));
    }
    console.log(element);
  }
    var content;
