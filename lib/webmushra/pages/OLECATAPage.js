...
function OLECATAPage(_pageManager, _pageTemplateRenderer, _pageConfig, _audioContext, _bufferSize, _audioFileLoader, _stimulus, _session, _errorHandler, _language) {
  ...
  for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
    ...
    if (element.type === "long_text") {
      ...
      for (let j = 0; j < element.response.length; ++j) {
        ...
        var label = $(
          "<label for='" + id + "'>" + "<span title='" + responseElement.popUp + "' style='text-decoration: none; color:black; font-weight: normal;'>"+ responseElement.label + "</span>"+ "</label>"
        );
        td2.append(checkbox);
        td2.append(label);
      }
    ...
    if (boxcount > 1) {
      console.log("Another textbox already exists on this page. Only the value of the last textbox on the page will be stored.");
    };
  };
  ...
  OLECATAPage.prototype.save = function () {
    ...
    for (let i = 0; i < this.pageConfig.questionnaire.length; ++i) { //loop through all questionnaire items (each item = element)
      var element = this.pageConfig.questionnaire[i];
      if (element.type == "checkboxes") {
        for (let response of element.response) { //loop through all attributes and add them to the attribute object
          var id_prefix = element.name + "__response";
