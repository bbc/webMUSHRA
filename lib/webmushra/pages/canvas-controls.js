...
class HeightSlider extends CanvasControl {
  ...
  render() {
    ...
    // scale markings
    var scaleMarkHalfWidth = this.scaleMarkWidth / 2;
  ...
  drawSources() {
    var sourceBlue = "rgba(0,0,150,0.75)";
    var sourceBlobSize = 10;
    this.drawSource(this.sourceParams["height"], sourceBlue, sourceBlobSize);
  }
  ...
  drawScale() {
    ...
    canvasContext.fillStyle = "black";
    var wordX = this.originX + this.scaleMarkWidth;
    canvasContext.fillText("At ear height", wordX, this.originY);
    canvasContext.fillText(
      "Much higher than ear height",
      wordX,
      this.originY - this.halfScaleHeight
    );
    canvasContext.fillText(
      "Slightly higher than ear height",
      wordX,
      this.originY - this.stepPix
    );
    canvasContext.fillText(
      "Slightly lower than ear height",
      wordX,
      this.originY + this.stepPix
    );
    canvasContext.fillText(
      "Much lower than ear height",
      wordX,
      this.originY + this.halfScaleHeight
    );
  }
...
class HeightSliderRel extends HeightSlider {
  constructor(_config) {
    super(_config);
    this.refHeight = 0;
  }
...
class AzRadRadar extends CanvasControl {
  ...
  render() {
    ...
    if (this.listenerImg !== null) {
      var imW = this.listenerImg.width * 0.8;
  ...
  drawSources() {
    ...
    var sourceBlue = "rgba(0,0,150,0.75)";
    var sourceBlobSize = 10;
    this.drawSource(this.sourceParams, sourceBlue, sourceBlobSize);
  }
  ...
  drawScale() {
    ...
    canvasContext.fillText(
      "Inside the head",
      wordX,
      this.originY - this.dStepPix * 0
    );
    canvasContext.fillText(
      "Very close",
      wordX,
      this.originY - this.dStepPix * 1
    );
    canvasContext.fillText("Close", wordX, this.originY - this.dStepPix * 2);
    canvasContext.fillText(
      "Quite close",
      wordX,
      this.originY - this.dStepPix * 3
    );
    canvasContext.fillText(
      "Mid distance",
      wordX,
    ...
    canvasContext.fillText(
      "Quite far",
      wordX,
      this.originY - this.dStepPix * 5
    );
    canvasContext.fillText("Far", wordX, this.originY - this.dStepPix * 6);
    canvasContext.fillText("Very far", wordX, this.originY - this.dStepPix * 7);
  }
  handleMouse(event) {
    this.calcMousePos(event);
    var storeResult = false;
...
class AzRadRadarRel extends AzRadRadar {
  constructor(_config) {
    super(_config);
    ...
    this.refParams = {
      ...
      distance: 4,
      width: _config.referenceParams.width || 0,
    };
  }
