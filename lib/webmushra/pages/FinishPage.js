...
function FinishPage(_pageManager, _session, _dataSender, _pageConfig, _language) {
  ...
  this.completionCode = Math.random().toString(36).substring(2,7);
...
FinishPage.prototype.storeParticipantData = function () {
  for (let i = 0; i < this.pageConfig.questionnaire.length; ++i) {
    ...
    if ($("#" + element.name).val()) {
      ...
      let chkbxs = $("input[name='" + element.name + "__response']:checked");
      this.session.participant.questionnaire[element.name] = chkbxs
        .map(function () {
          return this.id.replace(this.name+"_","");
        })
        .get()
        .join(",");
    }
  }
  if (this.pageConfig.showCompletionCode) {
    this.session.participant.completionCode = this.completionCode;
  }
};
...
FinishPage.prototype.render = function (_parent) {
  ...
  for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
    var element = this.pageConfig.questionnaire[i];
    var optionalText = (typeof(element.optional) == "undefined" || element.optional!==true) ? "<span style='color:red;'>*</span>" : "";
    if (element.type === "text") {
      table.append($("<tr><td><strong>" + element.label + "</strong>"+ optionalText+"</td><td><input id='" + element.name + "' /></td></tr>"));
    } else if (element.type === "number") {
      table.append($("<tr><td><strong>" + element.label + "</strong>"+ optionalText+"</td><td><input id='" + element.name + "' min='" + element.min + "' max='" + element.max + "' value='" + element.default + "' data-inline='true'/></td></tr>"));
    } else if (element.type === "likert") {
      this.likert = new LikertScale(element.response, element.name + "_");
      var td = $("<td></td>");
      table.append($("<tr></tr>").append(
        $("<td><strong>" + element.label + "</strong>"+ optionalText+"</td>"),
        td
      ));
      this.likert.render(td);
    } else if (element.type === "long_text") {
      table.append($("<tr><td id='labeltd' style='vertical-align:top; padding-top:" + $('#feedback').css('margin-top') + "'><strong>" + element.label + "</strong>"+ optionalText+"</td><td><textarea name='" + element.name + "' id='" + element.name + "'></textarea></td></tr>"));
    } else if (element.type === "checkboxes") {
      var tr = $("<tr></tr>");
      var td = $("<td><strong>" + element.label + "</strong>"+ optionalText+"</td>");
      table.append(tr);
      tr.append(td);
      var td2 = $("<td></td>");
  ...
  if (this.pageConfig.showCompletionCode) {
    var ucDiv = $("<div style='color:black; font-weight:bold;'>Your completion code is: " + this.completionCode + "</div>");
    $("#popupResultsContent").append(ucDiv);
  }
