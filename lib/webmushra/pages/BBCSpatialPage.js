    ...
    function (_event) {
      if (_event.name == "stopTriggered") {
        ...
        if ($("#buttonCondition").attr("active") == "true") {
          $.mobile.activePage
            .find("#buttonCondition") //remove color
            .removeClass("ui-btn-b")
            .addClass("ui-btn-a")
            .attr("data-theme", "a");
...
BBCSpatialPage.prototype.render = function (_parent) {
  ...
  tdSH.innerHTML =
    "<h3>Height</h3>Use the left mouse button to indicate the perceived source height (green) relative to the reference (blue).";
  trRatingHeading.appendChild(tdSH);

  var tdRH = document.createElement("td");
  tdRH.innerHTML =
    "<h3>Azimuth, Distance, and Width</h3>Use the left mouse button to indicate the perceived source azimuth and distance (green) relative to the reference (blue).<br/>Use the right mouse button to indicate the perceived source width (dragging up to increase it and down to decrease it).";
...
BBCSpatialPage.prototype.btnCallbackCondition = function () {
  ...
  if (
    ...
    this.cleanButtons();
    $.mobile.activePage
      .find("#buttonCondition") // add color to condition
      .removeClass("ui-btn-a")
      .addClass("ui-btn-b")
      .attr("data-theme", "b");
...
BBCSpatialPage.prototype.cleanButtons = function () {
  ...
  if ($("#buttonCondition").attr("active") == "true") {
    $.mobile.activePage
      .find("#buttonCondition") //remove color from condition
      .removeClass("ui-btn-b")
      .addClass("ui-btn-a")
      .attr("data-theme", "a");
    $("#buttonCondition").attr("active", "false");
