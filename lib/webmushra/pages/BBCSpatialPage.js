function BBCSpatialPage(_reference, _condition, _pageManager, _pageTemplateRenderer, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _errorHandler, _language) {
    ...
    this.setResult = (function (result) {
        ...
        // likert quality scale is set in save method
        this.checkCompleteResponse();
    }).bind(this);
    this.likertCallback = (function (prefix) {
        if (!prefix) return;
        this.result["timbral_quality"] = $("input[name='" + prefix + "_response']:checked").val();
        this.checkCompleteResponse();
    }).bind(this);
    this.checkCompleteResponse = (function () {
        if (this.result['azimuth'] !== null &&
            ...
            this.result['timbral_quality'] !== null)
...
BBCSpatialPage.prototype.initUI = function () {
    ...
    this.likert = new LikertScale(this.pageConfig.response, "timbral_quality", this.pageConfig.mustPlayback == true, this.likertCallback);
    // remove this?
...
BBCSpatialPage.prototype.render = function (_parent) {
    ...
    tdLH.innerHTML = "<h3>Timbral Quality</h3>Please indicate the quality of timbre of the test condition with respect to the reference."
    trRatingHeading.appendChild(tdLH);
    ...
    var tdL = $("<td></td>");
    this.likert.render(tdL);
    trRating.appendChild(tdL.get(0));
...
BBCSpatialPage.prototype.load = function () {
    ...
    if (this.result["timbral_quality"]) {
    ...
    // TODO: do I need to set anything else or does it get drawn automagically?
...
BBCSpatialPage.prototype.store = function (_reponsesStorage) {
    ...
    if (this.result == undefined) {
        ...
        rating.timbral_quality = this.result["timbral_quality"];
    }
