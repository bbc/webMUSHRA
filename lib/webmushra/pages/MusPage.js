...
MusPage.prototype.setConditionExtrasStatus = function(index, activate) {
  for (let i = 0; i < this.numExtraInputs; ++i) {
    ...
    if (activate === true)
      this.activateCheckbox(name, index);
    else
      this.deactivateCheckbox(name, index);
  }
};
...
MusPage.prototype.render = function (_parent) {
  ...
  for (let i = 0; i < this.numExtraInputs; ++i) {
    ...
    // scale spacer
    let tdConditionExtraScale = $("<td id='td_conditionExtraScale" + i + "'></td>");
    ...
    for (let j = 0; j < conditions.length; ++j) {
      ...
      let tdConditionExtra = $(
        "<td id='td_" + cb_id + "'></td>"
      );
      let checkbox = $("<input type='checkbox' id='" + cb_id + "' name = '" + extraInput.name + "'>");
      var label = $("<label for='" + cb_id + "'>" + extraInput.label + "</label>");
...
MusPage.prototype.renderScaleLabels = function (
  ...
  for (i = 0; i < scaleConfig.length; ++i) {
    var scaleLabelConfig = scaleConfig[i];
